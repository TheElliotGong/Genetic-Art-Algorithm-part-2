...
class VoronoiPainting:
    ...
    def _render_array(self, scale=1) -> np.ndarray:
        ...
        It first creates a binary image where the seed points are marked, then uses OpenCV's distance
        transform to compute the Voronoi regions. Finally, it maps the region labels to their
        corresponding colors and returns the resulting image as a NumPy array.
        """
    ...
    # This function adds an outline between the Voronoi regions to make the edges more distinct.
    # It uses the region labels to find boundaries and then dilates them to create a thicker line. The resulting image is returned as a NumPy array. The line width and outline color can be adjusted through the parameters.
    def _render_array_with_lines(
        ...
        """
        This function is similar to the `_render_array` function but adds configurable outline lines between
        the Voronoi regions to make the edges more distinct. It uses the region labels to find boundaries and
        then dilates them to create a thicker line. The resulting image is returned as a NumPy array.
        The line width and outline color can be adjusted through the parameters."""
        w = self._img_width * scale
        h = self._img_height * scale
        ...
        result[is_edge] = np.array(outline_color, dtype=np.uint8)

        return result

    # This function is similar to the `draw` function but adds configurable outline lines between the Voronoi regions to make the edges more distinct. The resulting image is returned as a PIL Image object. The line width and outline color can be adjusted through the parameters.
    def draw_lines(self, scale=1, line_width=2, outline_color=(0, 0, 0)) -> Image:
